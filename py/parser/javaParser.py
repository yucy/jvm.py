...
from struct import FieldInfo,MethodInfo,CClassFile,AttributeInfo
...
def javap(_data):
	...
	_cls_args.update({
		...
		'attribute_info':handlerAttr(attributes_count)
		})
...
def methodAndFieldHandler(_type,count):
	...
	for i in xrange(count):
		...
		args.update({
			'attributes_count':attributes_count,
			'attributes':handlerAttr(attributes_count)
			})
		temp.append(args)
	if _type == 'field':
...
def handlerAttr(count):
	attributes = []
	for x in xrange(count):
		attr_name = cursor(2)
		attr_length = cursor(4)
		attr_desc = cursor(getDecimal(attr_length))
		attr_arg = attr_name+attr_length+attr_desc
		attributes.append(AttributeInfo(attr_arg,constant_pool))
	return attributes
# 获取常量类型结构定义
